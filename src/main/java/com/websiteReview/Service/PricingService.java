...
public class PricingService {
    ...
    public PricingDto viewPricingById(int pricingId) {
        Pricing pricing = this.pricingRepository.findById(pricingId).orElseThrow(() -> new ResourceNotFoundException("The expected pricing is not found"));
        return this.modelMapper.map(pricing, PricingDto.class);
    }
    ...
    public List<PricingDto> getPricingBySoftware(int softwareId) {
        Software software = this.softwareRepository.findById(softwareId).orElseThrow(() -> new ResourceNotFoundException("The expected software is not found while fetching pricing"));
        List<Pricing> pricings = this.pricingRepository.findBySoftware(software);
        List<PricingDto> pricingDtos  = pricings.stream().map(pricing -> this.modelMapper.map(pricing, PricingDto.class)).collect(Collectors.toList());
        return pricingDtos;
    }
}
